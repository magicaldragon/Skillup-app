...
async function testLevels() {
  try {
    ...
    console.log('All levels:', allLevels.map(l => ({ id: l._id, name: l.name, code: l.code, isActive: l.isActive })));
    
    // Get active levels
    const activeLevelsData = await Level.find({ isActive: true });
    console.log('Active levels data:', activeLevelsData.map(l => ({ id: l._id, name: l.name, code: l.code })));
