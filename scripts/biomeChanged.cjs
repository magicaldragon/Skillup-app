...
const diff = spawnSync('git', ['diff', '--name-only', '--diff-filter=ACMRTUXB', `${baseRef}...HEAD`], { encoding: 'utf8' });
...
const res = spawnSync('npx', ['biome', 'check', '--max-diagnostics=200', '--reporter=json', ...files], { encoding: 'utf8' });
...
const threshRes = spawnSync('node', ['scripts/biomeThreshold.cjs', reportPath, String(threshold)], { stdio: 'inherit' });
