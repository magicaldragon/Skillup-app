...
const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({
  ...
  const studentScores = students
    ...
    .filter((s) => s.avg !== null);
  const top = [...studentScores].sort((a, b) => (b.avg || 0) - (a.avg || 0)).slice(0, 3);
  const bottom = [...studentScores].sort((a, b) => (a.avg || 0) - (b.avg || 0)).slice(0, 3);
  ...
  return (
    <div className="dashboard-card">
        ...
        {assignments.map((a) => (
          <div key={a.id} className="dashboard-section">
            ...
            <div className="dashboard-text">
              Avg Score: <b>{avgScore(a.id)}</b>
            </div>
            <div className="dashboard-text">
              Completion: <b>{completionRate(a.id)}</b>
            </div>
          </div>
        ))}
      </div>
      <div className="dashboard-section">
        ...
        {overdue.length === 0 ? (
          <div className="dashboard-text dashboard-muted">None</div>
        ) : (
