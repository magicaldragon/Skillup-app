  ...
  return (
    <div className="bg-white rounded-xl shadow p-6 max-w-5xl mx-auto mt-8">
      ...
      <div className="flex gap-4 mb-4">
        <select className="p-2 border rounded" value={filter.entityType} onChange={e => setFilter(f => ({ ...f, entityType: e.target.value }))}>
        ...
        <select className="p-2 border rounded" value={filter.action} onChange={e => setFilter(f => ({ ...f, action: e.target.value }))}>
        ...
        <input className="p-2 border rounded" placeholder="Filter by User ID..." value={filter.userId} onChange={e => setFilter(f => ({ ...f, userId: e.target.value }))} />
        <button className="px-4 py-2 bg-blue-600 text-white rounded" onClick={fetchLogs}>Refresh</button>
      </div>
      {loading ? (
            ...
            <tbody>
              {logs.map(log => (
                <tr key={log._id}>
                  ...
                  <td className="p-2">
                    <button className="px-3 py-1 bg-green-600 text-white rounded" onClick={() => setSelectedLog(log)}>View</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {selectedLog && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-2xl">
            ...
            <pre className="bg-slate-100 p-4 rounded text-sm overflow-x-auto max-h-96">{JSON.stringify(selectedLog, null, 2)}</pre>
            <div className="flex gap-2 justify-end mt-4">
              <button className="px-4 py-2 bg-gray-400 text-white rounded" onClick={() => setSelectedLog(null)}>Close</button>
            </div>
          </div>
        </div>
      )}
