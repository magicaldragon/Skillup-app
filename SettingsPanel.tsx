...
const SettingsPanel = ({ user, classes, onDataRefresh }: { user: Student, classes: StudentClass[], onDataRefresh?: () => void }) => {
  ...
  if (!editMode) {
    return (
      <div className="idcard-glass max-w-xl mx-auto mt-12 p-0 rounded-3xl shadow-2xl border-0 bg-gradient-to-br from-green-200/60 via-white/80 to-green-100/60 relative overflow-hidden transition-colors duration-500" style={{ minHeight: 420 }}>
        {/* Glassy background and accent ring */}
        <div className="absolute inset-0 bg-gradient-to-br from-green-200/60 via-white/80 to-green-100/60 backdrop-blur-xl z-0" />
        <div className="flex flex-col items-center gap-4 p-10 z-10 relative">
          {/* Avatar and badge */}
          <div className="relative mb-2">
            <div className="idcard-avatar-glass">
              {avatarUrl ? (
          ...
          {user.displayName && <div className="text-lg text-green-700 font-semibold mb-1">{user.displayName}</div>}
          <div className="flex flex-col gap-1 w-full max-w-md mt-2">
            <div className="flex justify-between text-base font-medium text-slate-700"><span>Email:</span><span>{user.email}</span></div>
            <div className="flex justify-between text-base font-medium text-slate-700"><span>Phone:</span><span>{form.phone || '—'}</span></div>
            <div className="flex justify-between text-base font-medium text-slate-700"><span>Date of Birth:</span><span>{form.dob || '—'}</span></div>
            {currentClasses.length > 0 && (
              <div className="flex justify-between text-base font-medium text-slate-700">
                <span>Current Classes:</span>
                <span>{currentClasses.join(', ')}</span>
              </div>
            )}
          </div>
  ...
  return (
    <div className="idcard-glass max-w-xl mx-auto mt-12 p-0 rounded-3xl shadow-2xl border-0 bg-gradient-to-br from-green-200/60 via-white/80 to-green-100/60 relative overflow-hidden transition-colors duration-500" style={{ minHeight: 420 }}>
      <div className="absolute inset-0 bg-gradient-to-br from-green-200/60 via-white/80 to-green-100/60 backdrop-blur-xl z-0" />
      <form onSubmit={handleSave} className="flex flex-col items-center gap-4 p-10 z-10 relative">
        <div className="relative mb-2">
          <div className="idcard-avatar-glass">
            {avatarUrl ? (
              <img src={avatarUrl} alt="User Avatar" className="w-32 h-32 rounded-full border-4 border-green-400 shadow-xl object-cover bg-slate-100" />
