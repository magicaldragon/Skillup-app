...
router.post('/', verifyToken, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const existingLevel = await admin.firestore()
      .collection('levels')
      .where('name', '==', name)
      .limit(1)
      .get();
    ...
    if (!levelOrder) {
      const lastLevel = await admin.firestore()
        .collection('levels')
        .orderBy('order', 'desc')
        .limit(1)
        .get();
...
router.delete('/:id', verifyToken, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const classesUsingLevel = await admin.firestore()
      .collection('classes')
      .where('levelId', '==', id)
      .limit(1)
      .get();
...
// Reorder levels
router.post('/reorder', verifyToken, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
