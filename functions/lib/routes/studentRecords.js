...
router.get('/', auth_1.verifyToken, async (req, res) => {
    ...
    try {
        ...
        let query = admin.firestore().collection('studentRecords');
        // Role-based filtering
        if (role === 'student') {
            // Students see only their own records
            query = query.where('studentId', '==', (_a = req.user) === null || _a === void 0 ? void 0 : _a.userId);
        }
            ...
            const teacherClasses = await admin
                .firestore()
                .collection('classes')
                .where('teacherId', '==', (_b = req.user) === null || _b === void 0 ? void 0 : _b.userId)
                .get();
        ...
        const studentRecords = snapshot.docs.map((doc) => (Object.assign({ id: doc.id }, doc.data())));
...
router.post('/', auth_1.verifyToken, auth_1.requireAdmin, async (req, res) => {
    var _a;
    try {
        const { studentId, classId, levelId, enrollmentDate, completionDate, status = 'enrolled', grade, attendance, notes, } = req.body;
        // Validate that student exists
        const studentDoc = await admin.firestore().collection('users').doc(studentId).get();
        if (!studentDoc.exists || ((_a = studentDoc.data()) === null || _a === void 0 ? void 0 : _a.role) !== 'student') {
        ...
        const studentRecordData = {
            ...
            enrollmentDate: enrollmentDate
                ? admin.firestore.Timestamp.fromDate(new Date(enrollmentDate))
                : admin.firestore.FieldValue.serverTimestamp(),
            completionDate: completionDate
                ? admin.firestore.Timestamp.fromDate(new Date(completionDate))
                : null,
            status,
            grade,
        ...
        const newStudentRecord = Object.assign({ id: docRef.id }, studentRecordData);
...
router.get('/:id', auth_1.verifyToken, async (req, res) => {
    ...
    try {
        ...
        if (role === 'student' && studentRecordData.studentId !== ((_a = req.user) === null || _a === void 0 ? void 0 : _a.userId)) {
        ...
        return res.json(Object.assign({ id: doc.id }, studentRecordData));
    }
    catch (error) {
...
router.get('/student/:studentId', auth_1.verifyToken, async (req, res) => {
    var _a, _b;
    try {
        const { studentId } = req.params;
        const { role } = req.user;
        // Check if user has access to this student's records
        if (role === 'student' && studentId !== ((_a = req.user) === null || _a === void 0 ? void 0 : _a.userId)) {
            return res.status(403).json({ message: 'Access denied' });
        }
        else if (role === 'teacher') {
            ...
            const classIds = (studentData === null || studentData === void 0 ? void 0 : studentData.classIds) || [];
            if (classIds.length === 0) {
                return res.json([]);
            }
            const teacherClasses = await admin
