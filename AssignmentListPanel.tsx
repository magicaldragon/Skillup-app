  ...
  return (
    <div className="bg-white rounded-xl shadow p-6 mb-8">
      <div className="flex flex-wrap gap-4 mb-4">
        <select value={classFilter} onChange={e => setClassFilter(e.target.value)} className="p-2 border rounded">
        ...
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search title..." className="p-2 border rounded flex-1" />
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody>
            {filtered.length === 0 && <tr><td colSpan={6} className="text-center text-slate-400">No assignments found.</td></tr>}
            {filtered.map(a => (
              <tr key={a.id}>
                ...
                <td className="p-2">{(a.classIds || []).map(cid => classes.find(c => c.id === cid)?.name).join(', ')}</td>
                <td className="p-2">
                  <button onClick={() => onSelectAssignment(a)} className="px-3 py-1 bg-blue-500 text-white rounded">View Submissions</button>
                </td>
              </tr>
            ))}
          </tbody>
